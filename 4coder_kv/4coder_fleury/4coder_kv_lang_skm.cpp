...
internal void
F4_Skm_ParseDeclSet(F4_Index_ParseCtx *ctx, F4_Index_Note *parent)
{
    ...
    for(;!ctx->done;)
    {
           ...
           F4_Index_RequireToken(ctx, S8Lit(":"), F4_Index_TokenSkipFlag_SkipWhitespace))
        {
            ...
            for(;!ctx->done;)
            {
                ...
                if(token->sub_kind == TokenSkmKind_Comma ||
...
internal void
F4_Skm_ParseDeclSet_Braces(F4_Index_ParseCtx *ctx, F4_Index_Note *parent)
{
    if(F4_Index_RequireToken(ctx, S8Lit("{"), F4_Index_TokenSkipFlag_SkipWhitespace))
    {
        F4_Skm_ParseDeclSet(ctx, parent);
        F4_Index_RequireToken(ctx, S8Lit("}"), F4_Index_TokenSkipFlag_SkipWhitespace);
    }
}

internal void
F4_Skm_ParseDeclSet_Parens(F4_Index_ParseCtx *ctx, F4_Index_Note *parent)
{
    if(F4_Index_RequireToken(ctx, S8Lit("("), F4_Index_TokenSkipFlag_SkipWhitespace))
    {
        F4_Skm_ParseDeclSet(ctx, parent);
        ...
        if(F4_Index_RequireTokenKind(ctx, TokenBaseKind_Identifier, &name, flags))
        {
            ...
            if(F4_Index_RequireToken(ctx, S8Lit("::"), flags))
            {
                    ...
                    F4_Index_PeekToken(ctx, S8Lit("("))))
                {
                    ...
                    F4_Skm_ParseDeclSet_Parens(ctx, parent);
                }
                // NOTE(rjf): Structs
                else if(F4_Index_RequireToken(ctx, S8Lit("struct"), flags))
                {
                    ...
                    F4_Skm_ParseDeclSet_Braces(ctx, parent);
                }
                // NOTE(rjf): Unions
...
internal Token *
_F4_Skm_FindDecl(Application_Links *app, Buffer_ID buffer, i64 pos, Token *decl_name)
{
    Token *result = 0;
    ...
    for(;;)
    {
        ...
        if(token)
        {
            ...
            else if(token->sub_kind == TokenSkmKind_BraceCl)
            {
                scope_nest += 1;
            }
            else if(token->sub_kind == TokenSkmKind_BraceOp)
            {
                scope_nest -= 1;
            }
        }
    ...
    // NOTE(rjf): Search for left parentheses (function call or macro invocation).
    {
        ...
        for(int i = 0; count < 4; i += 1)
        {
            ...
            if(token)
            {
                ...
                else if(token->sub_kind == TokenSkmKind_ParenOp)
                {
                    paren_nest -= 1;
                }
                else if(token->sub_kind == TokenSkmKind_ParenCl && i > 0)
                {
                    paren_nest += 1;
                }
                else if(token->sub_kind == TokenSkmKind_Comma && i > 0 && paren_nest == 0)
                {
                    arg_idx += 1;
                }
            }
        ...
        for(int i = 0; i < 3; i += 1)
        {
            ...
            if(token)
            {
                        ...
                        token_it_dec_non_whitespace(&it))
                {
                    ...
                    {
                        Token *decl_start = _F4_Skm_FindDecl(app, buffer, decl_name->pos, decl_name);
                        if(decl_start)
                        {
                            ...
                            for(;;)
                            {
                                ...
                                if(token2)
                                {
                                    if(token2->sub_kind == TokenSkmKind_Colon)
                                    {
                                        found_colon = 1;
                                    }
